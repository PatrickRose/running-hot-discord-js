...
export async function execute(interaction: CommandInteraction) {
    ...
    if (typeof roleName != 'string') {
        return interaction.reply({content: `\`${roleName}\` is not a valid facility name`, ephemeral: true});
    }

    await interaction.reply({content:`Setting ${roleName} as the role for ${isCorporation(corporation) ? CORPORATION_NAMES[corporation] : 'control'}`, ephemeral: true});
